...
woq_settings_default = {
    ...
    'ctcp'      : ('VERSION,USERINFO,SOURCE,FINGER', 'The ctcp requests to perform'),
...
def exec_command(buffer, nick):
    ...
    for ctcp_query in weechat.config_get_plugin('ctcp').split(','):
        weechat.command(buffer, "/ctcp {0} {1}".format(nick, ctcp_query))
